...
// proposes one new item at a time calibrated to that level, and lets the user make a card,
// mark it as already known, skip, or move on. All logic/state lives in App.jsx; this is
// presentational.
...
export default function DiscoverPanel(props) {
  ...
  return (
    <div>
      ...
      {/* Web-verify toggle */}
      <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: C.dim, marginBottom: 12, cursor: 'pointer' }}>
      ...
      {!suggestionLoading && !suggestion && !profileLoading && (
        <div style={{ fontSize: 12, color: C.dim, padding: '12px 0' }}>
          No suggestion yet. <button onClick={onNext} style={{ background: 'none', border: 'none', color: C.blue, cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, padding: 0, textDecoration: 'underline' }}>Get one</button>
        </div>
      )}
          ...
          {card ? (
            <div style={{ borderTop: '1px solid rgba(110,118,129,0.2)', paddingTop: 10, marginTop: 6 }}>
              ...
              <textarea value={card.front} onChange={(e) => setCard({ ...card, front: e.target.value })}
                style={{ width: '100%', background: 'rgba(0,0,0,0.2)', color: C.text, border: '1px solid rgba(110,118,129,0.25)', borderRadius: 4, padding: 8, fontSize: 12, fontFamily: 'inherit', resize: 'vertical', marginBottom: 8 }} rows={1} />
              ...
              <textarea value={card.back} onChange={(e) => setCard({ ...card, back: e.target.value })}
                style={{ width: '100%', background: 'rgba(0,0,0,0.2)', color: C.text, border: '1px solid rgba(110,118,129,0.25)', borderRadius: 4, padding: 8, fontSize: 12, fontFamily: 'inherit', resize: 'vertical', marginBottom: 8 }} rows={5} />
