...
QUALITY BAR: a good hook lets the learner RECONSTRUCT the answer from the hook alone${method === 'confuse' ? ' (for THIS method the test is: shown the confusable pair side by side, would the learner now pick the right one every time?)' : ''}. Mentally test yours: would someone who forgot this recover it from your hook? If not, try a different angle. Never output a vague "just associate X with Y". CLARITY: the hook must carry ONE unambiguous chain from cue to answer. Every sound-alike must map cleanly to its word part AND be AFFIRMED by the scene — never negated, questioned, or argued with (a line like "Just sell it? No!" teaches the learner "sell", or nothing). If a sound-alike only fits through a contortion, drop it and anchor that part a different way. Then EDIT FOR ECONOMY: the best hook is the SHORTEST one that STILL PASSES that test — one sharp image beats three decorations, so delete scene-setting, filler adjectives, and anything the learner does not need to replay to reach the answer. Trim FILLER, never PERSONALITY: the surprising, funny, vivid core is load-bearing — cutting it makes the hook forgettable. Re-run the test after trimming: if a cut makes the hook harder to recall, duller, or harder to reconstruct from, put the words back — a slightly longer hook that works beats a tight one that fails.
