...
// Generic flashcard generator for NON-language modes. The mode's own card design (description,
// back template, tag rules — all generated by createMode for the subject) is injected via the
// {DESCRIPTION}/{FORMAT}/{TAG_RULES} placeholders so every batch produces consistent cards.
// When the mode has no template, the model designs a format that helps the learner for that
// subject (chemistry ≠ vocabulary). Returns a JSON ARRAY.
export const GENERIC_CARD_PROMPT = `You generate study flashcards. Subject/mode: "{MODE}" ({TYPE}).{DESCRIPTION}
...
- "back": the card back as plain text, each labeled line on its OWN line. Write the LABELS in {LEARN_LANG}. Include these fields in order:
   ...
   • translation: the {USER_LANG} translation(s), ordered by how much THIS word is the go-to term for each meaning in {LEARN_LANG} (a meaning that a more common synonym dominates goes LAST, never first)
