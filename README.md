...
- **Ebi's memory hook** — every graded card has a **🧠 Help me remember this** button. Ebi builds a memory aid tailored to whatever you're studying: sound-alike + imagery hooks for vocabulary (e.g. Spanish *muelle* "dock" → picture a stubborn **mule** hauling cargo down at the dock), and acronyms / associations / mini-stories for concepts in non-language modes (CompTIA, music theory, etc.). "↻ Another hook" regenerates; the hook is written in your app language
- **Collapsible results** — graded cards (and the end-of-session Batch Results) collapse to a one-line header (word + rating), so after reviewing many cards you can scan the list and click any card to expand its questions, feedback, and memory hook. **Clear completed from list** sits at the very bottom so it's never mistaken for a continue/sync button
